...
\section{NutNet and DRAGNet group fieldwork}
\begin{enumerate}
\item The lab maintains one Nutrient Network field sites at Bunchgrass Meadow and one DRAGNet field site at Mt. Pisgah. Assisting with annual data collection should be prioritized by grads/postdocs and summer undergraduates.
\item DRAGNet sampling typically occurs over a three-day period in May. Mt. Pisgah is local, and the sampling is during the term, so this is an excellent opportunity to involve undergraduates interested in field work.
\item NutNet sampling typically occurs over a one-day period in mid to late July; typically we can complete this work without having to stay the night.
\item Our goals for the DRAGNet and NutNet sampling include: 1) Supporting and connecting to a larger network of ecologists, 2) Training newer members in field methods, 3) Building bonds among the lab, 4) Sustaining a relationship with local ecosystems. 
...
\begin{enumerate}
\item Six month plans are an opportunity to reflect on your short and long term goals (really they include six-month plans as well as one-year and five-year goals). You can use whatever structure works best for you - some people like to list items to accomplish and the anticipated time investment of each, others like to make calendars with sequential goals. They are stored in a shared lab google folder for continuity.  
